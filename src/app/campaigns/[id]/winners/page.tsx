...
export default function WinnersPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const winnersQuery = useQuery({
    queryKey: ['campaign-winners', id],
    queryFn: () => apiClient.get<Winner[]>(`/campaigns/${id}/winners`),
    enabled: !!campaignQuery.data,
  });

  if (campaignQuery.isLoading || winnersQuery.isLoading) {
    return (
  ...
  const hasError = campaignQuery.isError || winnersQuery.isError;
  ...
  if (hasError || !campaign) {
    return (
      <div className="p-10 text-center space-y-4">
        ...
        <Link href="/buyer/campaigns" className="text-[#1e3a8a] font-medium block">
          Back to campaigns
        </Link>
      </div>
    );
  ...
  const statusTone =
    campaign.status === CampaignStatus.DRAWN || campaign.status === CampaignStatus.COMPLETED
          ...
          <div className="text-center py-20 px-6 space-y-6 bg-white rounded-xl border border-[#e2e8f0] shadow-sm">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[#94a3b8] text-sm leading-relaxed">
                The winners have not been selected yet. Check back after the draw is completed.
              </p>
            </div>
              ...
              {winners.map((winner) => (
                <div
                  ...
                  className={`relative bg-white rounded-xl border border-[#e2e8f0] shadow-sm overflow-hidden p-5 flex items-center gap-4 ${
                    winner.prizeRank === 1
                      ? 'border-yellow-200 bg-gradient-to-br from-white to-yellow-50/30'
                      : ''
                  }`}
                >
                  <div
        ...
        <div className="bg-white p-4 rounded-xl border border-[#e2e8f0] shadow-sm flex gap-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-[#45464d] leading-relaxed">
              Public winner pages keep buyer identities masked while still showing the
              official ticket numbers selected during the draw.
            </p>
          </div>
        </div>
      </div>
